...
cur.execute(
    "CREATE TABLE IF NOT EXISTS students (id INTEGER PRIMARY KEY, name TEXT, roll_no INTEGER UNIQUE)"
)

cur.execute(
    "CREATE TABLE IF NOT EXISTS attendance (date TEXT, roll_no INTEGER, status TEXT, FOREIGN KEY(roll_no) REFERENCES students(roll_no))"
)
...
def mark_attendance(status):
    ...
    if date and selection:
        ...
        try:
            cur.execute(
                "INSERT INTO attendance (date, roll_no, status) VALUES (?, ?, ?)",
                (date, roll_no, status),
            )
            conn.commit()
            messagebox.showinfo(
                "Success",
                "Attendance marked as {} for roll number {}".format(status, roll_no),
            )
        except sql.IntegrityError:

            messagebox.showerror(
                "Error",
                f"Attendance already marked for roll number {roll_no} on {date}",
            )
    else:
...
present_button = tk.Button(
    att_frame, text="Present", command=lambda: mark_attendance("Present")
)
present_button.grid(row=2, column=0, padx=10, pady=10)

absent_button = tk.Button(
    att_frame, text="Absent", command=lambda: mark_attendance("Absent")
)
...
def view_attendance():
    ...
    if date:
        ...
        cur.execute(
            "SELECT students.name, students.roll_no, attendance.status FROM students INNER JOIN attendance ON students.roll_no = attendance.roll_no WHERE attendance.date = ?",
            (date,),
        )
